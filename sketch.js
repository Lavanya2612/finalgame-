...
function preload(){
  ...
  treasure2_img= loadImage('images/treasure_2.png')
...
monstergrp = new Group()
 ...
 coin24= createSprite(displayWidth/2 +300 ,displayHeight/2 +200,10,10)
   ...
   treasure=createSprite(displayWidth/2+600,displayHeight/2+300,150,150)
...
if(gameState===3){
  ...
  stroke("blue");
 textFont("Ink Free");
 text('oops..you could not find the lost treasure!',200,200)
  

 
}

if((gameState===2) && (coins===24)){
  gameState=4
  girl.x=displayWidth/2 -630
   girl.y=displayHeight/2 -300
...
if(gameState===4){
 ...
 textFont("Ink Free");
  ...
  if(girl.isTouching(monstergrp)){
    lives=lives-1
    girl.x=displayWidth/2 -630
    girl.y=displayHeight/2 -300
  }
  if(girl.isTouching(treasure)){
    gameState=5
    }
...
if(gameState===5){
  background(treasure2_img)
  fill('deeppink')
  stroke('orange')
  textSize(25)
  text('WOHOO...YOU ARE UNSTOPABBLE!! WAY TO GO!!',displayWidth/2-200,displayHeight/2)
}
